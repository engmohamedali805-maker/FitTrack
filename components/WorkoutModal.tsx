...
export const WorkoutModal: React.FC<WorkoutModalProps> = ({ isOpen, onClose, workoutData, onSave, dateStr }) => {
  ...
  const handleUpdateSet = (exerciseId: string, setId: string, field: keyof ExerciseSet, value: number) => {
    setExercises(prev => prev.map(ex => (ex.id !== exerciseId ? ex : { ...ex, sets: ex.sets.map(s => s.id === setId ? { ...s, [field]: value } : s) })));
  };

  const handleToggleSet = (exerciseId: string, setId: string) => {
    setExercises(prev => prev.map(ex => (ex.id !== exerciseId ? ex : { ...ex, sets: ex.sets.map(s => s.id === setId ? { ...s, completed: !s.completed } : s) })));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 h-screen w-screen flex flex-col bg-slate-50 overflow-hidden animate-in slide-in-from-bottom-full duration-300">
      <div className="bg-slate-900 px-4 py-4 flex justify-between items-center text-white shrink-0 shadow-md">
      ...
      {savingRoutineName !== null && (
        <div className="bg-indigo-600 p-4 text-white flex items-center gap-3 animate-in slide-in-from-top duration-300">
           <Layers size={20} /><input type="text" autoFocus placeholder="اسم الروتين الجديد..." className="bg-indigo-700 border-none rounded-lg px-3 py-2 flex-1 text-white placeholder:text-indigo-300 outline-none text-right" value={savingRoutineName} onChange={(e) => setSavingRoutineName(e.target.value)} />
              ...
              {expandedExerciseId === ex.id && (
                <div className="p-3 animate-in slide-in-from-top-2 duration-200">
                  <div className="grid grid-cols-12 gap-2 mb-2 px-2 text-[10px] font-black text-slate-400 uppercase text-center">
                    ...
                    {isCardio ? (<><div className="col-span-3">دقيقة</div><div className="col-span-3">سرعة</div><div className="col-span-3">ميل</div></>) : (<><div className="col-span-4">الوزن (kg)</div><div className="col-span-4">العدات</div></>)}
          ...
          <div className="p-4 border-b space-y-4 bg-slate-50">
            <div className="flex gap-2 items-center">
              ...
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="ابحث عن تمرين..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white rounded-2xl py-3.5 pl-10 pr-4 outline-none border border-slate-200 shadow-sm font-medium text-right" dir="rtl" />
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-white">
            {activeTab === 'templates' ? templates.map(t => (
              ...
              <button key={e.id} onClick={() => handleAddExercise(e)} className="w-full flex items-center justify-between p-4 border border-slate-100 rounded-2xl hover:bg-slate-50 transition-all group active:scale-[0.98]">
                <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-slate-50 p-2 group-hover:bg-white transition-colors"><MuscleIcon muscle={e.muscle}/></div><div className="text-right"><span className="font-bold text-slate-700 block text-sm">{e.name}</span><span className="text-[10px] text-slate-400 font-bold uppercase">{e.muscle}</span></div></div>
